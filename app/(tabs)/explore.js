// app/(tabs)/explore.js — Search Screen
...
export default function SearchScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {query.length >= 40 && (
        <Text style={styles.charCount}>{50 - query.length} characters remaining</Text>
      )}
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.hintText}>Please enter a valid search term</Text>
          <Text style={styles.hintSub}>Search cannot be empty or spaces only</Text>
        </View>
      ) : query.length === 0 ? (
        <View style={styles.center}>
          <Text style={styles.hintEmoji}>🍜</Text>
        ...
        <>
          <Text style={styles.resultCount}>{results.length} food item(s) found</Text>
          <FlatList
            data={results}
...
const styles = StyleSheet.create({
  ...
  searchWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', marginHorizontal: 16, marginTop: -16, borderRadius: 14, paddingHorizontal: 16, paddingVertical: 14, elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 6, gap: 10 },
  icon: { fontSize: 16 },
  input: { flex: 1, fontSize: 14, color: '#1a1a1a', padding: 0 },
  clear: { fontSize: 14, color: '#aaa', fontWeight: '700' },
  charCount: { fontSize: 11, color: '#FF6161', textAlign: 'right', paddingHorizontal: 20, paddingTop: 4, fontStyle: 'italic' },
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 8 },
  loadingText: { fontSize: 14, color: '#888', marginTop: 8 },
  hintEmoji: { fontSize: 52, marginBottom: 8 },
  hintText: { fontSize: 15, color: '#555', textAlign: 'center' },
  hintSub: { fontSize: 12, color: '#aaa', marginTop: 4 },
  resultCount: { fontSize: 13, color: '#888', fontStyle: 'italic', paddingHorizontal: 20, paddingTop: 14, paddingBottom: 4 },
});
...
// Defined directly here — no import needed
...
export default function FoodDetailScreen() {
  ...
  const handleFavourite = async () => {
    if (!item.available) {
      Alert.alert(
        '❌ Not Available',
        `"${item.name}" is currently not available and cannot be saved to favourites.`,
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    if (isFavourite) {
      ...
      try {
        ...
        if (result) {
          ...
          Alert.alert('❤️ Added!', `"${item.name}" added to Favourites!`, [{ text: 'OK' }]);
        }
      } catch (error) {
        setIsFavourite(true);
        Alert.alert('❤️ Added!', `"${item.name}" added to Favourites!`, [{ text: 'OK' }]);
        ...
        <View style={styles.content}>
          ...
          <View style={styles.infoRow}>
            ...
            <View>
              ...
              <Text style={styles.infoValue}>{item.location} • {item.distance}</Text>
            </View>
          </View>
          ...
          <View style={styles.infoRow}>
            ...
            <View>
              ...
              <Text style={[styles.infoValue, { color: catColor }]}>{item.category}</Text>
            </View>
          </View>
          ...
          <TouchableOpacity
            style={[styles.favBtn, {
              backgroundColor: !item.available ? '#CCCCCC' : isFavourite ? '#c94040' : '#FF6161'
            }]}
          ...
          {!item.available && (
            <Text style={styles.unavailNote}>⚠️ This item is currently not available</Text>
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  backBtn: { position: 'absolute', top: 16, left: 16, backgroundColor: 'rgba(0,0,0,0.45)', width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  ...
  unavailBadge: { position: 'absolute', bottom: 12, left: 12, backgroundColor: '#333', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 20 },
  unavailText: { color: '#fff', fontSize: 11, fontWeight: '600' },
  content: { padding: 20 },
  catBadge: { alignSelf: 'flex-start', paddingHorizontal: 14, paddingVertical: 5, borderRadius: 20, marginBottom: 12 },
  ...
  name: { fontSize: 24, fontWeight: '800', color: '#1a1a1a', flex: 1, marginRight: 10 },
  price: { fontSize: 22, fontWeight: '800', color: '#FF6161' },
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 16 },
  star: { fontSize: 14, fontWeight: '600', color: '#555' },
  reviews: { fontSize: 13, color: '#888' },
  dotView: { width: 4, height: 4, borderRadius: 2, backgroundColor: '#ccc' },
  ...
  descTitle: { fontSize: 16, fontWeight: '700', color: '#1a1a1a', marginBottom: 8 },
  desc: { fontSize: 14, color: '#555', lineHeight: 22, marginBottom: 20 },
  apiInfoBox: { backgroundColor: '#1a1a1a', borderRadius: 12, padding: 14, marginBottom: 16, gap: 8 },
  apiInfoTitle: { color: '#fff', fontSize: 13, fontWeight: '700', marginBottom: 4 },
  apiRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  getBadge: { backgroundColor: '#27AE60', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  postBadge: { backgroundColor: '#2980B9', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  badgeText: { color: '#fff', fontSize: 10, fontWeight: '800', letterSpacing: 1 },
  apiText: { color: '#aaa', fontSize: 12 },
  favBtn: { borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginBottom: 10 },
  postingRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  favBtnText: { color: '#fff', fontSize: 16, fontWeight: '700' },
  unavailNote: { textAlign: 'center', fontSize: 13, color: '#FF6161', marginTop: 4 },
});